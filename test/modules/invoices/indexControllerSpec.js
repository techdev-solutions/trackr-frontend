define(['baseTestSetup'], function(baseTestSetup) {
    ...
    describe('invoice index controller', function() {
        ...
        it('loadInvoices uses the search loader if searchQuery is set', inject(function($httpBackend) {
            ...
            $httpBackend.expectGET(/^api\/invoices\/search\/findByIdentifierLikeAndInvoiceState\?identifier=%25test%25&page=\d+&projection=\w+&size=\d+&sort=creationDate&state=OVERDUE/);
            $httpBackend.expectGET(/^api\/invoices\/search\/findByIdentifierLikeAndInvoiceState\?identifier=%25test%25&page=\d+&projection=\w+&size=\d+&sort=creationDate&state=OUTSTANDING/);
            $httpBackend.expectGET(/^api\/invoices\/search\/findByIdentifierLikeAndInvoiceState\?identifier=%25test%25&page=\d+&projection=\w+&size=\d+&sort=creationDate&state=PAID/);
            $httpBackend.flush();
        }));
